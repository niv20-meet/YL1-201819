...
try:
        s = socket.socket()
        host = '192.168.252.172' #enter ip address
except:
        pen.clear()
...
def movearound():
	...
	message = struct.pack(">{}l".format(number_of_balls * 3 + 6), *all_info) 
	all_info = []
...
while True:
        while running == True:
                ...
                try:  #resiving data 
                        inpt = c.recv(1024)
                        inpt = struct.unpack(">{}l".format(3), inpt)
                        in_x,in_y,in_r = inpt
        ...
        message = struct.pack(">{}l".format(30), *death_message)
